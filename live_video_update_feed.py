...
def pick_commercial_visual_scene():
    scenes = [
        ...
        "A detailed close-up of compact drain-cleaning tools (portable auger or inspection camera) clearing a multifamily drain line near apartment service access points.",
...
def build_gemini_video_prompt(title, description, cta):
    ...
    prompt_parts = [
        ...
        "Coverage requirement: use mostly close-up and medium-close shots of real plumbing actions: snaking, compact auger use, trap removal, shutoff checks, drain flow testing, camera inspection, and cleanup verification.",
        ...
        "Technician appearance: if a person is shown, they must appear as an apartment service plumber in practical service attire (work pants, plain work shirt, gloves when task-appropriate).",
        "Uniform constraints: clothing must be plain and unbranded with no names, no identifying logos, and no company branding.",
        "Wardrobe exclusions: do NOT use hard hats, high-visibility construction vests, or heavy construction PPE unless a clearly hazardous scene requires it.",
